...
Given a set of $k$ actions, $\mathcal{A}$, and number of rounds $T$. Repeat for $t$ in $T$ rounds:
\begin{enumerate}
    \item Algorithm selects arm $A_t \in \mathcal{A}$
    \item Algorithm observes reward $R_t \in [0,1]$
\end{enumerate}
Goal: maximise expected total reward. $q_* (a) = \mathbb{E}[R_t|A_t = a]$

\textbf{Non-stationary Probability Distributions}

$N_t(a)$ is the number of times action $a$ is observed.
$$Q_t (a) = \frac{1}{N_t (a)}\sum_{\tau=1}^{t-1} R_\tau \cdot \mathbb{1}_{A_t = a} \quad \quad
     \lim_{N_t(a) \rightarrow \infty} Q_t (a) = q_* (a)$$

Greedy actions are selected with probability $1-\epsilon$

\textbf{Greedy Action Selection} $A_t = A^*_t = \arg \max_a Q_t(a)$

\textbf{Random Action Selection} $A_t \sim \text{Unif}(\mathcal{A})$

\textbf{Sample Average} $Q_n = \frac{\sum_i^{n-1} R_i}{n-1}$. The sample average can be updated incrementally to avoid recomputation such that $Q_{n+1} = Q_n + \frac{1}{n} [R_n  - Q_n] $

\textbf{Stochastic Approximation Convergence ($\alpha$-reduction)} Estimates $Q_n$ will converge with probability 1 to $q_*$ if: $$\sum_{n=1}^\infty \alpha_n (a) \rightarrow \infty \quad \text{ and } \quad \sum_{n=1}^\infty \alpha^2_n (a) < \infty$$

Valid: $\alpha_n = \frac{1}{n}$; Invalid: $\alpha_n = c, \alpha_n = \frac{1}{n^2}$

\subsubsection{Upper Confidence Bound (UCB)}
$$A_t = \begin{cases}
    & \text{Unif}(\mathcal{A}) \quad \max_a N_t(a) = 0 \\
    & \arg \max_a [Q_t (a) + c \sqrt{\frac{\log t}{N_t (a)}}] \text{ else }
\end{cases}$$

\subsection{Markov Decision Process (MDPs) $(\mathcal{S, A}, r, p, \gamma)$}
\textbf{Backup Diagrams} diagram the relationships that form the basis of \textit{backup} operations which transfer value information back to a state (or state-action pair) form its successor states (or state-action pairs). A finite \textbf{Markov Decision Process}, MDP, is defined by:
...
\subsection{Return}
% In RL the goal of the agent is not the maximization of the immediate reward, but of the cumulative reward in the long run.
...
For any optimal policy: $\forall \hat{\pi} \forall s: v_\pi (s) \geq v_{\hat{\pi}} (s) $
...
Monte Carlo (MC) methods require only experience from a real or simulated environment.


\subsection{Policy Improvement Theorem}
Using a greedy policy meets conditions for policy improvement theorem.

\begin{align*}
    & q_{\pi_k}(s, \pi_{k+1}(s)) = q_{\pi_k}(s, \arg \max_a q_{\pi_k}(s, a)) \\
    & = \max_a q_{\pi_k}(s,a) \geq q_{\pi_k}(s, \pi_k(s)) = v_{\pi_k}(s)
\end{align*}
...
Given episodes generated from $\mu$, the expected return at $t$ is $\mathbb{E}_\mu[G_t | S_t=s] = v_\mu(s)$. We use the importance sampling ratio to obtain $v_\pi(s) = \mathbb{E}_\mu[\rho_{t:T} G_t | S_t=s]$$$\rho_{t:T} := \prod_{k=t}^{T-1} \frac{\pi(A_k |S_k)}{\mu(A_k|S_k)}$$ 

\textit{Note:} The importance sampling ratio is prone to numerical underflow and takes time to estimate
\begin{algorithm}[H]
 Initialise:
...
We can instead use an $\epsilon$-soft policy where $\pi(a|s) \geq \epsilon/|\mathcal{A}|$ for $\epsilon > 0$. We improve the policy by making it $\epsilon$-greedy wrt $q_\pi$. This is performed after updating the value function such that 
...
\begin{algorithm}[H]
   \ForEach{$s$ in the episode:}{
        ...
        \For{all $a \in \mathcal{A}(s)$} {
            $\pi'(a|s) := \begin{cases}
    & \epsilon/|\mathcal{A}|+ (1-\epsilon) \text{ if } a = \arg \max_{a'} q_\pi (s, a') \\ & \epsilon/|\mathcal{A}| \text{ otherwise}  
\end{cases}$
        }
   } 
...
\begin{align*}
    ...
    & q_\pi(s, a) \approx \eta^{-1} \sum_{t_i \in \mathcal{E}(s)} \rho_{t_{i+1}:T_i} G_{t_i}^i
\end{align*}

\textbf{Ordinary} $\eta = |\mathcal{E}(s,a)|$ \\

\textbf{Weighted} $\eta = \sum_{t_i \in \mathcal{E}(s)} \rho_{t_{i(+1)}: T_i}$
...
\subsection{On/Off-Policy Learning with $n$-step Returns}
$$Q_{t+n} (S_t, A_t) := Q_{t+n-1} (S_t, A_t) + \alpha \rho_{t+1:t+n} [G_{t:t+n} - Q_{t+n-1}(S_t, A_t)]$$ where $$\rho_{t:h} = \prod_{k=t}^{\min(h,T-1)} \frac{\pi (A_k|S_k)}{\mu (A_k | S_k)}$$
...
\begin{center}
\includegraphics[width=0.55\linewidth]{./images/Planning-Learning.png}
\end{center}
...
\begin{center}
\includegraphics[width=0.7\linewidth]{./images/Dyna.png}
\end{center}



% Also the model-learning process is in background. As new information is gained, the model is updated to better match reality. As the model changes, the ongoing planning process will gradually compute a different way of behaving to match the new model.\\
...
\For{$t=0,1,2...$ do}{
    ...
    \For{$n$ rollouts do}{
        \While{$S$ is non-terminal (or fixed-length rollouts) do}{
        select action $A$ based on $Q(S,\cdot)$ with some exploration \\
    ...
    Select action $A_t$ greedily from $Q(S_t, \cdot)$
}
...
\For{$t=0,1,2...$ do}{
    ...
    \For{$n$ rollouts do}{
        ...
        select action $A$ based on $Q(S,\cdot)$ with some exploration \\
        $(R,S') \sim \text{Model}(S,A)$ \\
        push $(S,A,R,S')$ to Trace \\
        $S \leftarrow S'$ 
    } 
    \while{Trace not empty do}{
        pop $(S,A,R,S')$ from Trace: \\
        $Q(S,A) \leftarrow Q(S,A) + \alpha [R + \gamma \max_a Q(S', a) - Q(S,A)]$
    }
    }
...
\begin{algorithm}[H]
    ...
    \While{within computational budget}{
        ...
        $\triangle \leftarrow \text{DefaultPolicy}(\text{state}(v_l))$ \\
        $\text{Backprop}(v_l, \triangle)$
    }
...
State-Return Estimates $(S_t, U_t)$:
\begin{itemize}
    \item MC: $U_t = G_t$
    \item $TD(0)$: $U_t = R_{t+1} + \gamma \hat{v}(S_{t+1}, w_t)$
    \item $TD(n)$: $U_t = \sum_{k=1}^n \gamma^{k-1} R_{t+k} + \gamma^n V_{t+n-1}(S_{t+1}, w_{t+n-1})$
    \item Sarsa: $U_t = R_{t+1} + \gamma \hat{q}(S_{t+1}, A_{t+1}, w_t$
    \item Expected Sarsa: $U_t = R_{t+1} + \gamma \sum_a \pi(a|S_{t+1}) \hat{q}(S_{t+1}, a, w_t$
    \item Q-learning: $U_t = R_{t+1} + \gamma \max_a \hat{q}(S_{t+1}, a, w_t)$
\end{itemize}
...
Let $J(w) = \mathbb{E}_\pi[(v_\pi(s) - \hat{v}(s,w))^2]$ be a differentiable function of $w$. Convergence requires standard $\alpha$-reduction.

$$w_{t+1} = w_t + \frac{1}{2} \alpha \nabla J(w_t)$$

\subsubsection{Gradient Bandit Algorithm}
Let $\pi_t(a|\theta) = Pr\{A_t = a|\theta_t = \theta\}$ be a differentiable policy. We use gradient ascent to maximise the expected reward $$\theta_{t+1} = \theta_t + \alpha \nabla_{\theta_t} \mathbb{E}[R_t]$$

\textbf{Example: Gradient Bandit Algorithm with Softmax}

$H_t(a)$ denotes preference values for actions.

\begin{align*}
    & \pi_t(a) = \frac{e^{H_t(a)}}{\sum_b e^{H_t(b)}} \\
    & H_{t+1}(a) = H_t(a) + \alpha (R_t - \bar{R_t})([a=A_t]_1 - \pi_t(a)) \\ 
    & \bar{R}_t = \frac{1}{t} \sum_{\tau=1}^t R_\tau = \bar{R}_{t-1} + \frac{1}{t}(R_t - \bar{R}_{t-1})
\end{align*}
...
\ForEach{step of episode}{
    Take action $A$, observe $R, S'$ \\
    \If{$S'$ is terminal}{
        $w \mathrel{+}= \alpha[R - \hat{q}(S,A,w)]\nabla \hat{q}(S,A,w)$ \\
        Go to next episode
    }
    Choose $A'$ as function of $\hat{q}(S', \cdot, w)$ (e.g. $\epsilon$-greedy) \\
    $w \mathrel{+}= \alpha[R - \gamma \hat{q}(S',A',w) - \hat{q}(S,A,w)]\nabla \hat{q}(S,A,w)$ \\
...
\hline
Algorithm                                                                      & Tabular & Linear                                                      & \begin{tabular}[c]{@{}c@{}}Non\\ Linear\end{tabular} \\ \hline
MC Control                                                                     & yes     & \begin{tabular}[c]{@{}c@{}}oscillates \\ near*\end{tabular} & no                                                   \\ \hline
\begin{tabular}[c]{@{}c@{}}(semi-gradient)\\ n-step Sarsa\end{tabular}         & yes     & \begin{tabular}[c]{@{}c@{}}oscillates \\ near*\end{tabular} & no                                                   \\ \hline
\begin{tabular}[c]{@{}c@{}}(semi-gradient)\\ n-step \\ Q-learning\end{tabular} & yes     & no                                                          & no                                                   \\ \hline
\end{tabular}
\caption{Convergence to Global Optimum in Episodic Control}
\end{table}
...
\textbf{Softmax Policy} $$\pi(a|s, \theta) = \frac{e^{h(s,a,\theta)}}{\sum_{b \in \mathcal{A}}e^{h(s,b,\theta)}}, \quad h(s,a,\theta) = \theta^\top x(s,a)$$
...
\While{forever (for each episode)}{
Initialise $S$ (first state of episode); $I \leftarrow 1$ \\
\While{S is not terminal (for each timestep)}{
    $A \sim \pi(\cdot|S,\theta)$ \\
    Take action $A$, observe $S', R$ \\
    $\delta \leftarrow R + \gamma \hat{v}(S', w) - \hat{v}(S,w) \quad$ if $S'$ is terminal, then $\hat{v}(S', w) := 0$ \\
...
\begin{center}
\includegraphics[angle=90,width=0.7\linewidth]{./images/Tabular_Value_Function.png}
\end{center}
\end{multicols}
